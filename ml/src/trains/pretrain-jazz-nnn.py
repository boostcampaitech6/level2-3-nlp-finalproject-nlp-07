...
if deterministic: # c`udnn random seed 고정 - 고정 시 학습 속도가 느려질 수 있습니다. 
...
def main():
    # change root path
    project_path = "/home/level2-3-nlp-finalproject-nlp-07/ml"
    ...
    midi_paths = load_midi_paths(args_data["Jazz"])
    ...
    train_dataset = CodeplayDataset(midis=train_midi_chunks, min_seq_len=50, max_seq_len=args["max_seq_len"]-2, tokenizer=tokenizer, use_meta=False)
    valid_dataset = CodeplayDataset(midis=valid_midi_chunks, min_seq_len=50, max_seq_len=args["max_seq_len"]-2, tokenizer=tokenizer, use_meta=False)
    ...
    output_path = project_path+f"/models/pretrain/jazz-nnn-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
        
    steps = 400
    # Commented parameters correspond to the small model
    trainer_config = {
        "output_dir": output_path,
        "num_train_epochs": 50, # 학습 epoch 자유롭게 변경. 저는 30 epoch 걸어놓고 early stopping 했습니다.
